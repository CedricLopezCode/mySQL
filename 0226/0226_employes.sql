...
SELECT * FROM employes HAVING COMM; #correction
#2
SELECT DNOM, ENO, SAL FROM employes order by ENO, SAL DESC;
#3
...
#7
SELECT MIN(AVG(SAL)) FROM employe GROUP BY PROF LIMIT 1;
#8
SELECT MIN(SAL) FROM employe GROUP BY PROF LIMIT 1;
#9
